...
def create_graph() -> nx.Graph:
    ...
    for row in gdf.itertuples():
        G.add_node(row.nom_barri, codi_barri=row.codi_barri) # Node attributes: nom_barri and codi_barri 
        
    sindex = gdf.sindex # Create spatial index

    for i, barri in gdf.geometry.items():
        candidates = list(sindex.intersection(barri.bounds)) # Intersection aproximation using bounding boxes
        for j in candidates:
            if i < j and barri.intersects(gdf.geometry[j]): # Check for exact intersections
