...
export async function getDailyDataForMonth(day: string) {
  const result: QueryResult<any> = await pool.query(
    `
    select d.day, d.precipitation_sum, d.is_holiday, d.temperature_2m_max, d.temperature_2m_min, SUM(d.intensity) as tt
    from display_data d
    where d.day LIKE '` +
      getMonth(day) +
      `-%'
    group by d.day, d.precipitation_sum, d.is_holiday, d.temperature_2m_max, d.temperature_2m_min
    `,
  );
  return result.rows;
}
...
export async function getMapDataForMonth(day: any) {
  const result: QueryResult<any> = await pool.query(
    `
    with z as
    (select barri, stddev_samp(intensity) as std, avg(intensity) as mean
    from display_data
    group by barri)
    select d.day, d.barri, d.intensity, (d.intensity - z.mean) / z.std as zscore
    from display_data d, z
    where d.day LIKE '` +
      getMonth(day) +
      `-%' and z.barri = d.barri
...
export async function getTableDataForMonth(day: any) {
  const result: QueryResult<any> = await pool.query(
    `
    with z as
    (select barri, stddev_samp(intensity) as std, avg(intensity) as mean
    from display_data
    group by barri)
    select d.day, d.barri, d.intensity, z.mean, d.intensity / g.superficie as density, (d.intensity - z.mean) / z.std as zscore
    from display_data d, geospatial_data g, z
    where d.barri = z.barri and g.nom_barri = d.barri and d.day LIKE '` +
      getMonth(day) +
      `-%'
    `,
  );
