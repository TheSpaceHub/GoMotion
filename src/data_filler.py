...
def add_weather_features(
    ...
    if end_string > ONE_WEEK.strftime("%Y-%m-%d"):
        ...
        weather_df = pd.concat(
            [
                daily_weather_summary(start=start_string, end=yesterday_string),
                weather_forecast_1_week(end=end_string),
            ]
        )
    else:
    ...
    # renaming
    df.loc[df["day"] > date_from, "temperature_2m_max"] = df["temperature_2m_max (°C)"]
    df.loc[df["day"] > date_from, "temperature_2m_min"] = df["temperature_2m_min (°C)"]
    df.loc[df["day"] > date_from, "precipitation_sum"] = df["precipitation_sum (mm)"]
